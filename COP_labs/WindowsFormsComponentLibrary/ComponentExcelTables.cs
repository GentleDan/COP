...
{
    public partial class ComponentExcelTables : Component
    {
        public ComponentExcelTables()
        {
            InitializeComponent();
        }
        public ComponentExcelTables(IContainer container)
        {
            container.Add(this);
        ...
        public bool CreateDoc(ExcelInfo info)
        {
            if (string.IsNullOrEmpty(info.FileName) || string.IsNullOrEmpty(info.Title))
            {
                throw new Exception("Не задан путь до файла или загoловок");
            }
            if (info.Data == null)
            {
